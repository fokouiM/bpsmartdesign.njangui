  ...
  <div class="d-flex flex-wrap align-items-start">
      ...
      <div class="flex"
          ...
          <small class="text-100">
              <strong><?= isset($old) ? 'Modification' : 'Ajout' ?> d'un nouveau dépot.</strong><br>
              <span>Cette interface vous permet <?= isset($old) ? 'de modifier' : 'd\'ajouter' ?>  un nouveau dépot.</span>
          </small>
      </div>
  </div>
</div>
          ...
          <form action="save" method="post">
            <div class="was-validated">
                <div class="form-row">
                  ...
                  <div class="col-12 col-md-12 mb-3">
                      <label class="form-label" for="validationSample01">Montant déposé</label>
                      <?php if(isset($old)) : ?>
                        <input type="text" class="form-control" name="montant" id="validationSample01" value="<?= (int)$old->montant ?> " required="">
                      <?php else : ?>
                        <input type="number" class="form-control" name="montant" id="validationSample01" placeholder="Entrer le montant déposé" required="">
                      <?php endif; ?>
                  ...
                  <div class="col-12 col-md-12 mb-3">
                      <label class="form-label" for="validationSample01">Date de depot</label>
                      <?php if(isset($old)) : ?>
                        <input type="text" class="form-control" name="date" id="validationSample01" value="<?= $old->date ?> " required="" >
                      <?php else : ?>
                        <input type="date" class="form-control" name="date" id="validationSample01" placeholder="Date de depot" required="">
                      <?php endif; ?>
                      <div class="invalid-feedback">Veuillez renseigner la date de depot.</div>
                      <div class="valid-feedback">Parfait!</div>
                  </div>
                </div>
            </div>
